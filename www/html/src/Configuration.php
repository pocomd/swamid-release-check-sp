  ...
  public function __construct($startDB = true) {
    ...
    $defaultValuesFederation = array(
      ...
      'instructionsAttributes' => '<p>' . _('Click on the green button to see what attributes your Identity Provider releases.') . '</p>
          <p>' . _('Description of all test available in the eduGAIN test suite') . ':
            <ul>
              <li>' . _('The Attributes tab shows all attributes the service release to the entityId https://release-check.<org>.<tld>/shibboleth. The entityId uses the entity categories') . ':<ul>
              ...
              <li>' . _('The MFA tab checks if an Identity Provider is correctly configured for handling request for multi-factor login.') . '</li>
